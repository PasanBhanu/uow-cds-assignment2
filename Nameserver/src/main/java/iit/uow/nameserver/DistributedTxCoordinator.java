...
public class DistributedTxCoordinator extends DistributedTx{
    ...
    public boolean perform() throws KeeperException, InterruptedException {
        ...
        for (String path : childrenNodePaths) {
            path = currentTransaction + "/" + path; System.out.println("Checking path :" + path);
            data = client.getData(path, false); String dataString = new String(data); if (!VOTE_COMMIT.equals(dataString)) {
    ...
    public void sendGlobalCommit() throws KeeperException, InterruptedException {
        if (currentTransaction != null) { System.out.println("Sending global commit for " + currentTransaction);
